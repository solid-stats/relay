...
const envSchema = z.object({
  ...
  RELAY_RATE_LIMIT_PER_MINUTE: z.coerce.number().int().min(1).max(100000).default(600),
  RELAY_TOKEN_SECRET: z.string().min(32),
  RELAY_TOKEN_TTL_DAYS: z.coerce.number().int().min(1).max(3650).default(30),
  RELAY_ADMIN_RATE_LIMIT_PER_MINUTE: z.coerce.number().int().min(1).max(1000).default(30),
...
const parseAdminEmails = (rawAdminEmails) => new Set(
  rawAdminEmails
    .split(',')
...
export const config = {
  ...
  TRUSTED_AUTH_EMAIL_HEADER: parsedEnv.data.TRUSTED_AUTH_EMAIL_HEADER.toLowerCase(),
  TRUSTED_AUTH_USER_HEADER: parsedEnv.data.TRUSTED_AUTH_USER_HEADER.toLowerCase(),
  TRUSTED_AUTH_SECRET_HEADER: parsedEnv.data.TRUSTED_AUTH_SECRET_HEADER.toLowerCase()
};
